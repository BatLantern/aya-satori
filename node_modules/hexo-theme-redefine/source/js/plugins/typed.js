...
export const config = {
  ...
  usrHitokotoCache: theme.home_banner.subtitle.cache, 
};
...
export default function initTyped(id) {
  ...
  async function getHitokoto() {
    ...
    try {
      const response = await fetch(usrHitokotoAPI);
  ...
  if (theme.home_banner.subtitle.hitokoto.enable) {
    getHitokoto().then((data) => {
      if (data) {
        if (data.from && theme.home_banner.subtitle.hitokoto.show_from) {
          typing("『" + data.hitokoto + "』—— 出自「" + data.from + "」");
        } 
        else {
