...
export default function initUtils() {
  const utils = {
    ...
    updateHomeBannerBlur() {
      ...
      if (theme.home_banner.style === "fixed" && (location.pathname === config.root || location.pathname.startsWith(config.root + 'page/'))) {
    ...
    initPageHeightHandle() {
      ...
      if (allDomHeight < innerHeight) {
        const marginTopValue = Math.min(Math.floor(innerHeight - allDomHeight), 7);
        if (marginTopValue > 0) {
