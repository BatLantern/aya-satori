            ...
            <div class="article-header flex flex-row gap-2 items-center px-2 sm:px-6 md:px-8">
                ...
                <div class="info flex flex-col justify-between">
                    <div class="author flex items-center">
                        <% let authorName = (typeof page.author === "string" && page.author) || page.author?.name || theme.info.author || config.author %>
                        <span class="name text-default-text-color text-lg font-semibold"><%= authorName %></span>
                        <% if (theme.hasOwnProperty('articles') && theme.articles.author_label.enable === true) { %>
                            <span class="author-label">
                                <% if(theme.articles.author_label.auto){ %>
                                    <span class="label-item ml-1.5 text-xs px-2 py-0.5"><%- getAuthorLabel(site.posts.length, theme.articles.author_label.auto, theme.articles.author_label.list) %></span>
                                <% } else if(theme.articles.author_label.sync === 'one'){ %>
                                    <span class="label-item label-item-1 ml-1.5 text-xs px-2 py-0.5"> <%= theme.articles.author_label.list[0] %></span>
                                <% } else { %>
                                    <% theme.articles.author_label.list.forEach(function(label, index) { %>
                                        <span class="label-item label-item-<%= index + 1 %> ml-1.5 text-xs px-2 py-0.5"><%= label %></span>
                                    <% }); %>
                                <% } %>
                            </span>
                        <% } %>
